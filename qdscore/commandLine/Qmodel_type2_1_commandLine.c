...
void usage(void) {
	...
	printf(" * XML source: Qmodel_type2_1.xml\n");
	printf(" * STATA dta time stamp: 2 Sep 2009 08:55\n");
	printf(" * C file create date: Mon Jun 14 10:39:39 BST 2010\n");
	printf(" *\n");
	printf("Usage:\n");
	printf("  Qmodel_type2_1_commandLine age b_corticosteroids b_cvd b_treatedhyp bmi ethrisk fh_diab smok surv town\n");
}
...
int main (int argc, char *argv[]) {
	...
	double score = type2_male(age,b_corticosteroids,b_cvd,b_treatedhyp,bmi,ethrisk,fh_diab,smok,surv,town,&error,errorBuf,sizeof(errorBuf));
	if (error) {
		printf("%s", errorBuf);
		exit(1);
	}
