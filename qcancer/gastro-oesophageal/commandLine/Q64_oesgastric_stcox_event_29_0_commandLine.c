 ...
 * C file create date: Thu  3 Nov 2011 11:49:20 GMT
 */
...
void usage(void) {
	...
	printf(" * XML source: Q64_oesgastric_stcox_event_29_0.xml\n");
	printf(" * STATA dta time stamp: 18 May 2011 12:50\n");
	printf(" * C file create date: Thu  3 Nov 2011 11:49:20 GMT\n");
	printf(" *\n");
	printf("Usage:\n");
	printf("  Q64_oesgastric_stcox_event_29_0_commandLine age c_hb new_abdopain new_appetiteloss new_dysphagia new_gibleed new_weightloss smoke_cat surv\n");
}
...
int main (int argc, char *argv[]) {
	...
	double score = oesgastric_female(age,c_hb,new_abdopain,new_appetiteloss,new_dysphagia,new_gibleed,new_weightloss,smoke_cat,surv,&error,errorBuf,sizeof(errorBuf));
	if (error) {
		printf("%s", errorBuf);
		exit(1);
	}
