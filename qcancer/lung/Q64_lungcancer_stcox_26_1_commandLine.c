...
void usage(void) {
	...
	printf(" * XML: Q64_lungcancer_stcox_event_26_1.xml\n");
	printf(" * STATA dta time stamp: 6 Mar 2011 16:20\n");
	printf(" * C file create date: Thu  3 Nov 2011 12:33:41 GMT\n");
	printf(" *\n");
	printf("Usage:\n");
	printf("  Q64_lungcancer_stcox_26_1_commandLine b_copd bmi c_hb new_appetiteloss new_haemoptysis new_weightloss s1_cough smoke_cat town age noOfFollowupYears\n");
}
...
int main (int argc, char *argv[]) {
	...
	int b_copd = atoi(argv[1]);
	double bmi = atof(argv[2]);
	int c_hb = atoi(argv[3]);
	int new_appetiteloss = atoi(argv[4]);
	int new_haemoptysis = atoi(argv[5]);
	int new_weightloss = atoi(argv[6]);
	int s1_cough = atoi(argv[7]);
	int smoke_cat = atoi(argv[8]);
	double town = atof(argv[9]);
	int age = atoi(argv[10]);
	int noOfFollowupYears = atoi(argv[11]);


        lifetimeRiskInit("Q64_lungcancer_stcox_time_26_1");

	double a_event = Q64_lungcancer_stcox_event_26_1_event(b_copd,bmi,c_hb,new_appetiteloss,new_haemoptysis,new_weightloss,s1_cough,smoke_cat,town,&error,errorBuf,sizeof(errorBuf));
	if (error) {
		printf("%s", errorBuf);
		exit(1);
	}
	double a_death = Q64_lungcancer_stcox_death_26_1_death(b_copd,bmi,c_hb,new_appetiteloss,new_haemoptysis,new_weightloss,s1_cough,smoke_cat,town,&error,errorBuf,sizeof(errorBuf));
	if (error) {
		printf("%s", errorBuf);
		exit(1);
	}
