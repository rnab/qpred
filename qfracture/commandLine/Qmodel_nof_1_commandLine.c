...
void usage(void) {
	...
	printf(" * XML source: Qmodel_nof_1.xml\n");
	printf(" * STATA dta time stamp: 2 Sep 2009 08:55\n");
	printf(" * C file create date: Fri Nov 20 22:53:03 GMT 2009\n");
	printf(" *\n");
	printf("Usage:\n");
	printf("  Qmodel_nof_1_commandLine age b_asthma b_corticosteroids b_cvd b_falls b_liver b_ra b_tca b_type2 bmi close_alcohol_category smoke_cat surv\n");
}
...
int main (int argc, char *argv[]) {
	...
	double score = nof_male(age,b_asthma,b_corticosteroids,b_cvd,b_falls,b_liver,b_ra,b_tca,b_type2,bmi,close_alcohol_category,smoke_cat,surv,&error,errorBuf,sizeof(errorBuf));
	if (error) {
		printf("%s", errorBuf);
		exit(1);
	}
