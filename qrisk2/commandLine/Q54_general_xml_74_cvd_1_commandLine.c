...
void usage(void) {
	...
	printf(" * XML source: Q54_general_xml_74_cvd_1.xml\n");
	printf(" * STATA dta time stamp: 26 Feb 2010 13:37\n");
	printf(" * C file create date: Fri Apr 16 11:10:47 BST 2010\n");
	printf(" *\n");
	printf("Usage:\n");
	printf("  Q54_general_xml_74_cvd_1_commandLine age b_AF b_ra b_renal b_treatedhyp b_type2 bmi ethrisk fh_cvd rati sbp smok surv town\n");
}
...
int main (int argc, char *argv[]) {
	...
	double score = cvd_male(age,b_AF,b_ra,b_renal,b_treatedhyp,b_type2,bmi,ethrisk,fh_cvd,rati,sbp,smok,surv,town,&error,errorBuf,sizeof(errorBuf));
	if (error) {
		printf("%s", errorBuf);
		exit(1);
	}
